...
class pySaliencyMap:
    ...
    def OFMGetFM(self, src):
        ...
        # convoluting a Gabor filter with an intensity image
        #  to extract oriemtation features
        ...
        for j in range(2, 9):
            GaborOutput0.append(cv2.filter2D(
                GaussianI[j], cv2.CV_32F, self.GaborKernel0))
            GaborOutput45.append(cv2.filter2D(
                GaussianI[j], cv2.CV_32F, self.GaborKernel45))
            GaborOutput90.append(cv2.filter2D(
                GaussianI[j], cv2.CV_32F, self.GaborKernel90))
            GaborOutput135.append(cv2.filter2D(
                GaussianI[j], cv2.CV_32F, self.GaborKernel135))
...
#            sys.exit("size mismatch")
        ...
        R, G, B, img = self.SMExtractRGBI(src)
        # extracting feature maps
        IFM = self.IFMGetFM(img)
        CFM_RG, CFM_BY = self.CFMGetFM(R, G, B)
        OFM = self.OFMGetFM(img)
        MFM_X, MFM_Y = self.MFMGetFM(img)
        ...
        self.SM = cv2.resize(smoothedSM, (width, height),
